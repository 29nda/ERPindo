...
export const api = {
  ...
  updateSettings: (
    ...
    input: {
      ...
      npwp?: string;
      logoDataUrl?: string;
      autoClosingEntry?: boolean;
      /** Profil pajak (Fase 22e) — menentukan tenggat mana yang muncul di kalender. */
      pkp?: boolean;
  ...
  pettyCash: (tenantId: string) => request<ApiKasKecil>("GET", `/api/tenants/${tenantId}/petty-cash`),
  setPettyCashImprest: (tenantId: string, danaTetap: number) =>
    request<ApiKasKecil>("PATCH", `/api/tenants/${tenantId}/petty-cash`, { danaTetap }),
  /** Jumlah pengisian TIDAK dikirim klien — server menghitungnya dari saldo buku besar. */
  pettyCashReplenish: (tenantId: string, input: { sourceAccountId: string; entryDate: string }) =>
    request<{ ok: true; jumlah: number; entryNo: string; status: ApiKasKecil | null }>(
      "POST",
      `/api/tenants/${tenantId}/petty-cash/replenish`,
      input,
    ),
  pettyCashCount: (tenantId: string, input: { hitunganFisik: number; entryDate: string; note?: string }) =>
    request<{ ok: true; selisih: number; arah: "kurang" | "lebih" | "pas"; entryNo: string | null; status: ApiKasKecil | null }>(
      "POST",
      `/api/tenants/${tenantId}/petty-cash/count`,
      input,
    ),
  ...
  /** Proyeksi arus kas 30/60/90 hari (Fase 22f) — BACA-SAJA, tidak menjurnal apa pun. */
  cashProjection: (tenantId: string) =>
    request<ProyeksiArusKas & { hariIni: string; arus: ArusDiproyeksikan[] }>(
      "GET",
      `/api/tenants/${tenantId}/reports/cash-projection`,
    ),
  ...
  taxCalendar: (tenantId: string) =>
    request<{
      hariIni: string;
      profil: { pkp: boolean; pphFinalUmkm: boolean; badan: boolean; adaKaryawan: boolean };
      tenggat: TenggatPajak[];
    }>("GET", `/api/tenants/${tenantId}/tax/calendar`),

  // --- Metode & kelompok pajak per aset (Fase 22d) --------------------------
  setAssetTax: (
    tenantId: string,
    id: string,
    input: { depreciationMethod?: MetodePenyusutan; taxGroup?: string | null; taxMethod?: MetodePenyusutan | null },
  ) => request<{ ok: true }>("PATCH", `/api/tenants/${tenantId}/assets/${id}/tax`, input),
  /** Laporan BACA-SAJA: angka fiskal tidak pernah dijurnal. */
  taxDepreciation: (tenantId: string, year: number) =>
    request<{
      tahun: string;
    ...
    }>("GET", `/api/tenants/${tenantId}/assets/tax-depreciation?year=${year}`),
  ...
  /** Revaluasi saldo valas akhir periode (Fase 22a) — jurnal + pembaliknya. */
  forexRevaluation: (tenantId: string, asOf: string) =>
    request<{
      ok: true;
      entryNo: string;
    ...
    }>("POST", `/api/tenants/${tenantId}/forex-revaluation`, { asOf }),
  ...
  createProductionOrder: (
    tenantId: string,
    input: { productId: string; warehouseId: string; qty: number; labourCost?: number; overheadCost?: number },
  ) =>
  ...
  listItems: <T = Record<string, unknown>>(
    tenantId: string,
    entity: "products" | "contacts" | "warehouses" | "price-groups",
    opts?: ListOpts,
  ) => request<{ items: T[]; total: number }>("GET", `/api/tenants/${tenantId}/${entity}${listQs(opts)}`),
  createItem: (
    tenantId: string,
    entity: "products" | "contacts" | "warehouses" | "price-groups",
    input: ProductInput | ContactInput | WarehouseInput | PriceGroupInput,
  ...
  archiveItem: (tenantId: string, entity: "products" | "contacts" | "warehouses" | "price-groups", id: string) =>
    request<{ ok: true }>("POST", `/api/tenants/${tenantId}/${entity}/${id}/archive`),
  // --- Daftar harga per grup pelanggan (Fase 23a) -------------------------------
  priceGroupItems: (tenantId: string, groupId: string) =>
    request<{ items: ApiPriceGroupItem[] }>(
      "GET",
      `/api/tenants/${tenantId}/price-groups/${groupId}/items`,
    ),
  setPriceGroupItem: (tenantId: string, groupId: string, productId: string, unitPrice: number) =>
    request<{ ok: true }>(
      "PUT",
      `/api/tenants/${tenantId}/price-groups/${groupId}/items/${productId}`,
      { unitPrice },
    ),
  // Menghapus baris, bukan menyetelnya 0: di daftar harga, 0 berarti gratis.
  clearPriceGroupItem: (tenantId: string, groupId: string, productId: string) =>
    request<{ ok: true }>(
      "DELETE",
      `/api/tenants/${tenantId}/price-groups/${groupId}/items/${productId}`,
    ),
  resolvePrices: (tenantId: string, contactId: string) =>
    request<{ groupId: string | null; groupName: string | null; prices: Record<string, number> }>(
      "GET",
      `/api/tenants/${tenantId}/price-groups/resolve?contactId=${encodeURIComponent(contactId)}`,
    ),
  ...
  // Form lead publik (Fase 21e) — khusus Pemilik.
  leadForm: (tenantId: string) =>
    request<{ token: string | null; slug: string }>("GET", `/api/tenants/${tenantId}/lead-form`),
  rotateLeadFormToken: (tenantId: string) =>
    request<{ ok: true; token: string; slug: string }>("POST", `/api/tenants/${tenantId}/lead-form/token`),
  disableLeadForm: (tenantId: string) =>
    request<{ ok: true }>("DELETE", `/api/tenants/${tenantId}/lead-form/token`),
