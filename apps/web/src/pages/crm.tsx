 ...
 *
 * Cuplikannya sengaja HTML polos tanpa JavaScript: pemilik UKM menempelkannya
 * di pembuat landing page apa pun (Carrd, Wix, Blogger) yang umumnya melarang
 * skrip pihak ketiga. `<form method="post">` biasa akan memindahkan pengunjung
 * ke halaman JSON, jadi dipakai `fetch` sekecil mungkin — dan itu satu-satunya
 * skrip di dalamnya.
 */
function KartuFormLead({ tenantId }: { tenantId: string }) {
  const u = useUi();
  const toast = useToast();
  const queryClient = useQueryClient();
  const formQuery = useQuery({
    queryKey: ["lead-form", tenantId],
    queryFn: () => api.leadForm(tenantId),
  });

  const rotate = useMutation({
    mutationFn: () => api.rotateLeadFormToken(tenantId),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["lead-form", tenantId] }),
    onError: (err) => toast("error", (err as Error).message),
  });
  const disable = useMutation({
    mutationFn: () => api.disableLeadForm(tenantId),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["lead-form", tenantId] }),
    onError: (err) => toast("error", (err as Error).message),
  });
  ...
  const cuplikan =
    data?.token && typeof window !== "undefined"
      ? `<form onsubmit="event.preventDefault();fetch('${window.location.origin}/api/form/lead/${data.slug}',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token:'${data.token}',name:this.name.value,email:this.email.value,phone:this.phone.value,message:this.message.value})}).then(()=>this.reset())">
  <input name="name" placeholder="${u("phFormNama")}" required />
  <input name="email" type="email" placeholder="${u("phFormEmail")}" />
  <input name="phone" placeholder="${u("phFormHp")}" />
  <textarea name="message" placeholder="${u("phFormPesan")}"></textarea>
  <button type="submit">${u("phFormKirim")}</button>
</form>`
      : "";

  return (
    <Card>
      <CardHeader title={u("formLeadPublik")} description={u("descFormLeadPublik")} />
          ...
          <>
            ...
            <div className="flex flex-wrap gap-2">
              <Button
                variant="secondary"
                onClick={() => {
                  navigator.clipboard?.writeText(cuplikan);
                  toast("success", u("salinCuplikan"));
                }}
              >
                {u("salinCuplikan")}
              </Button>
...
type DraftLine = {
  ...
  qty: string;
  unitPrice: string;
  /** Harga diketik manual (Fase 23b) — ganti pelanggan tidak menimpanya. */
...
export function QuotationsPage() {
  ...
  function pickProduct(i: number, productId: string) {
    ...
    setLine(i, {
      productId,
      unitPrice: p ? String(hasil.harga || "") : "",
      hargaDisentuh: false,
  ...
  /**
   * Ganti pelanggan → muat daftar harga grupnya, hitung ulang baris yang belum
   * diketik manual. Penawaran yang nanti dikonversi jadi faktur memakai harga
   * yang TERSIMPAN di penawaran — itu harga yang sudah dijanjikan ke pelanggan,
   * jadi konversinya sengaja tidak menghitung ulang apa pun.
   */
            ...
            <div className="grid gap-3 sm:grid-cols-4">
              <div>
                ...
                <Select
                  id="q-contact"
                  value={contactId}
                  onChange={(e) => void pilihPelanggan(e.target.value)}
                >
                ...
                {grupHarga.nama ? (
                  <div className="mt-1" data-testid="quo-grup-aktif">
                    <Badge tone="green">
              ...
              {lines.map((line, i) => (
                ...
                >
                  ...
                  <Input
                    ...
                    onChange={(e) => setLine(i, { unitPrice: e.target.value, hargaDisentuh: true })}
                  />
