 ...
 *
 * Yang ditampilkan hanya kewajiban yang BERLAKU bagi perusahaan ini — daftar
 * yang memuat PPN untuk non-PKP melatih orang mengabaikan seluruh kalendernya.
 * Profilnya karena itu disetel di sini juga, bukan disembunyikan di Pengaturan:
 * kalender yang salah paling mudah diperbaiki di tempat ia terlihat salah.
 */
function KalenderPajakSection({ isAdmin }: { isAdmin: boolean }) {
  const { tenant } = useWorkspace();
  const u = useUi();
  const toast = useToast();
  const queryClient = useQueryClient();
  ...
  const kalender = useQuery({
    queryKey: ["tax-calendar", tenant.tenantId],
    queryFn: () => api.taxCalendar(tenant.tenantId),
  });

  const simpanProfil = useMutation({
    mutationFn: (patch: { pkp?: boolean; pphFinalUmkm?: boolean; badanUsaha?: boolean }) =>
      api.updateSettings(tenant.tenantId, patch),
    onSuccess: () => {
      toast("success", u("kpProfilDisimpan"));
      queryClient.invalidateQueries({ queryKey: ["tax-calendar", tenant.tenantId] });
      queryClient.invalidateQueries({ queryKey: ["notifications", tenant.tenantId] });
    },
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader title={u("kpProfilJudul")} description={u("kpProfilDesc")} />
        <CardBody>
          {kalender.isLoading || !profil ? (
            <Spinner />
          ) : (
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3" data-uji="kp-profil">
              {(
                [
                  ["pkp", u("kpPkp"), profil.pkp],
                  ["pphFinalUmkm", u("kpPphFinalUmkm"), profil.pphFinalUmkm],
                  ["badanUsaha", u("kpBadanUsaha"), profil.badan],
                ] as const
              ).map(([key, label, nilai]) => (
      ...
      <Card>
        <CardHeader title={u("kpJudul")} description={u("kpDesc")} />
        <CardBody>
          {kalender.isLoading ? (
            <Spinner />
                ...
                {(kalender.data?.tenggat ?? []).map((t) => (
                  <Tr key={`${t.jenis}-${t.kegiatan}-${t.masa}`}>
                    ...
                    <Td label={u("kpTenggat")} className="whitespace-nowrap">
                      {t.tanggal}
                      {t.tanggal !== t.tanggalUu ? ` (${u("kpDigeser")})` : ""}
                    </Td>
                    <Td numeric label={u("kpSisaHari")}>
                      {t.sisaHari < 0 ? (
                        <Badge tone="red">
                          {u("kpTerlambat")} {-t.sisaHari}
                        </Badge>
                      ) : t.sisaHari <= 7 ? (
                        <Badge tone="amber">{t.sisaHari}</Badge>
                      ) : (
                        t.sisaHari
                      )}
                    </Td>
