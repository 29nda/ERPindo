...
import { GRUP_HARGA_KOSONG, hargaBaris, useGrupHarga } from "../lib/hargaGrup";
      ...
      <CardHeader
        ...
        action={
          <Button variant="secondary" onClick={() => setOpen((v) => !v)}>
            {open ? u("tutupRekap") : u("lihatRekap")}
          </Button>
        }
      />
      {open ? (
...
export function PosPage() {
  ...
  // --- Harga bertingkat di kasir (Fase 23b) --------------------------------
  //
  // Kasir TIDAK punya pemilih pelanggan — `routes/pos.ts` memakai kontak tetap
  // "Pelanggan Umum" — jadi yang dipilih di sini adalah GRUP harganya. Lebih
  // sedikit ketukan di konter, dan memang begitu cara toko grosir/ecer bekerja.
  //
  // Bawaan sengaja "tanpa grup" (harga dasar): kasir yang lupa menurunkan mode
  // grosir akan menjual sehari penuh di bawah harga, jadi keadaan bawaan harus
  // yang paling tidak merugikan.
  const [grupId, setGrupId] = useState("");
  const grupHarga = useGrupHarga(tenant.tenantId);
  const grupQuery = useQuery({
    queryKey: ["price-groups", tenant.tenantId],
    queryFn: () =>
      api.listItems<{ id: string; name: string }>(tenant.tenantId, "price-groups", { limit: 200 }),
  });
  const grupList = grupQuery.data?.items ?? [];
  ...
  const sale = useMutation({
    ...
    onSuccess: (res) => {
      ...
      setCart([]);
      setTenders([]);
      // Grup direset tiap transaksi selesai, BUKAN bertahan sepanjang shift:
      // `posSaleSchema` tidak menyimpan grup, jadi transaksi yang keliru dijual
      // pada harga grosir tidak meninggalkan jejak apa pun untuk ditelusuri.
      setGrupId("");
      grupHarga.bersihkan();
      invalidateShift();
  ...
  function recallHeld(h: {
    ...
    // Transaksi yang ditahan dipanggil lagi pada harga SAAT DITAHAN — tidak
    // dihitung ulang. Penalaran sama dengan "Ubah faktur" (Fase 10c): itulah
    // harga yang sudah disebutkan ke pelanggan. Grupnya ikut direset supaya
    // layar tidak mengaku memakai grup yang harga di keranjang tak mencerminkannya.
    setGrupId("");
    grupHarga.bersihkan();
  ...
  function addToCart(p: ProductRow) {
    // Satuan sengaja tidak dilewatkan: kasir selalu menjual per satuan dasar,
    // dan daftar harga memang disimpan per satuan dasar (migrasi 0046).
    const harga = hargaBaris(grupHarga.harga, { productId: p.id, hargaDasar: p.sell_price }).harga;
    setCart((c) => {
      const existing = c.find((i) => i.productId === p.id);
      if (existing) return c.map((i) => (i.productId === p.id ? { ...i, qty: i.qty + 1 } : i));
      return [...c, { productId: p.id, name: p.name, unitPrice: harga, qty: 1, discountPct: 0 }];
    });
  }

  /**
   * Ganti grup harga → hitung ulang SELURUH keranjang.
   *
   * Aman menghitung ulang semuanya karena harga satuan **tidak bisa diketik
   * manual di kasir** (hanya qty & diskon yang bisa disunting). Kalau suatu
   * hari POS bisa menyunting harga, aturan ini WAJIB ikut berubah — kalau
   * tidak, harga nego di konter akan hilang tanpa jejak.
   */
  async function gantiGrup(idBaru: string) {
    setGrupId(idBaru);
    const nama = grupList.find((g) => g.id === idBaru)?.name ?? null;
    const { harga } = idBaru
      ? await grupHarga.muatGrup(idBaru, nama)
      : (grupHarga.bersihkan(), GRUP_HARGA_KOSONG);

    const dasar = new Map(
      (productsQuery.data?.items ?? []).map((p) => [(p as ProductRow).id, (p as ProductRow).sell_price]),
    );
    setCart((c) =>
      c.map((i) => {
        const hargaDasar = dasar.get(i.productId);
        // Produk yang tak ada di halaman hasil pencarian saat ini dibiarkan apa
        // adanya: menebak harga dasarnya dari harga keranjang akan salah persis
        // pada barang yang sedang berharga khusus.
        if (hargaDasar === undefined) return i;
        return { ...i, unitPrice: hargaBaris(harga, { productId: i.productId, hargaDasar }).harga };
      }),
    );
  }
  ...
  const PESAN_PINDAI: Record<SebabGagalPindai, UiKey> = {
    "pengurai-gagal": "pindaiPenguraiGagal",
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_24rem]">
        ...
        <Card>
          <CardBody className="space-y-3">
            {/* Pemilih grup harga (Fase 23b). Ditaruh di ATAS pencarian produk,
                bukan di panel keranjang: ia menentukan harga setiap barang yang
                nanti diklik, jadi harus terbaca sebelum barang pertama masuk. */}
            {grupList.length > 0 ? (
              <div className="flex items-center gap-2">
                <Label htmlFor="pos-grup" className="mb-0 shrink-0">
                  {u("ghGrup")}
                </Label>
                <Select
                  id="pos-grup"
                  className="flex-1"
                  value={grupId}
                  onChange={(e) => void gantiGrup(e.target.value)}
                >
                ...
                {/* `data-testid` di WRAPPER, bukan di <Badge>: komponen itu
                    hanya menerima `tone` & `children`, jadi atribut lain
                    dibuang diam-diam (kelas cacat yang sama dengan Fase 17b). */}
                {grupHarga.nama ? (
                  <span data-testid="pos-grup-aktif">
                    <Badge tone="green">{grupHarga.nama}</Badge>
                  </span>
                ) : null}
              </div>
            ) : null}
            ...
            <div className="grid max-h-[60vh] grid-cols-2 gap-2 overflow-y-auto sm:grid-cols-3 xl:grid-cols-4">
              {products.map((p) => (
                ...
                >
                  ...
                  <div
                    data-testid={`pos-harga-${p.sku}`}
                    className="mt-1 text-sm font-semibold tabular-nums"
                  >
                    {formatIDR(
                      hargaBaris(grupHarga.harga, { productId: p.id, hargaDasar: p.sell_price }).harga,
                    )}
                  </div>
                </button>
              ))}
        ...
        <Card>
          ...
          <CardBody className="space-y-3">
            ...
            >
              {sale.isPending ? <Spinner /> : null} {u("bayarCetakStruk")}
            </Button>
