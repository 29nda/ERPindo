...
type DraftLine = {
  ...
  qty: string;
  unitPrice: string;
  /** Harga diketik manual (Fase 23b) — ganti pelanggan tidak boleh menimpanya. */
  hargaDisentuh: boolean;
  /** Harga berasal dari daftar harga grup pelanggan (Fase 23b). */
...
export function ContractsPage() {
  ...
  async function fetchProductOptions(q: string) {
    ...
    return res.items.map((p) => ({
      value: p.id,
      label: `${p.sku} · ${p.name}`,
      hint: formatIDR(hargaBaris(grupHarga.harga, { productId: p.id, hargaDasar: p.sell_price }).harga),
    }));
  }
  ...
  /**
   * Ganti pelanggan → muat daftar harga grupnya dan hitung ulang baris yang
   * harganya belum diketik manual. Faktur berulang yang diterbitkan cron nanti
   * memakai harga yang TERSIMPAN di kontrak, bukan daftar harga hari itu —
   * kontrak adalah harga yang disepakati untuk seluruh masanya.
   */
  async function pilihPelanggan(opt: { value: string; label: string }) {
    setForm((f) => ({ ...f, contactId: opt.value, contactLabel: opt.label }));
    const { harga } = await grupHarga.muat(opt.value);
    setLines((ls) =>
      ls.map((l) => {
        if (!l.productId || l.hargaDisentuh) return l;
        const p = productCache.current.get(l.productId);
        if (!p) return l;
            ...
            <div className="grid gap-3 sm:grid-cols-4">
              ...
              <div>
                ...
                <SearchSelect
                  ...
                  onSelect={(opt) => void pilihPelanggan(opt)}
                />
                {grupHarga.nama ? (
                  <div className="mt-1" data-testid="ct-grup-aktif">
                    <Badge tone="green">
