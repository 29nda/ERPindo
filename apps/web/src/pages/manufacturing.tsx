...
export function ManufacturingPage() {
  ...
  // Fase 21f — biaya konversi sebatch (bukan per unit): pemilik UKM menghitung
  // "upah tukang hari ini Rp 300rb untuk 200 bungkus", bukan per bungkus.
  const [ordTenaga, setOrdTenaga] = useState("");
  const [ordOverhead, setOrdOverhead] = useState("");
  ...
  const createOrder = useMutation({
    mutationFn: () =>
      api.createProductionOrder(tenant.tenantId, {
        ...
        qty: Number(ordQty) || 0,
        labourCost: Number(ordTenaga) || 0,
        overheadCost: Number(ordOverhead) || 0,
      }),
    onSuccess: () => {
          ...
          <Card>
            ...
            <CardBody className="space-y-4">
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                <div>
                  <Label htmlFor="ord-tenaga">{u("biayaTenagaKerjaBatch")}</Label>
                  <Input
                    id="ord-tenaga"
                    type="number"
                    min={0}
                    placeholder="0"
                    value={ordTenaga}
                    onChange={(e) => setOrdTenaga(e.target.value)}
                  />
                </div>
                <div>
                  <Label htmlFor="ord-overhead">{u("biayaOverheadBatch")}</Label>
                  <Input
                    id="ord-overhead"
                    type="number"
                    min={0}
                    placeholder="0"
                    value={ordOverhead}
                    onChange={(e) => setOrdOverhead(e.target.value)}
                  />
                </div>
                  ...
                  <Tr key={o.id}>
                    ...
                    <Td label={u("status")}>
                      <Badge tone={o.status === "produced" ? "green" : "neutral"}>
                        {o.status === "produced" ? u("statusSelesai") : u("statusDraf")}
                      </Badge>
                    </Td>
                    <Td label="QC">
  ...
  return (
    <Card>
      ...
      <CardBody className="space-y-4">
        ...
        >
          <div>
            <Label htmlFor="wc-code">{u("kode")}</Label>
            <Input
              id="wc-code"
              value={code}
          ...
          <div>
            <Label htmlFor="wc-rate">{u("tarifPerJam")}</Label>
            <Input
              id="wc-rate"
              type="number"
              min={0}
...
function RoutingCard({ orders, isAdmin }: { orders: ApiProductionOrder[]; isAdmin: boolean }) {
  ...
  return (
    <Card>
      ...
      <CardBody className="space-y-4">
        <div>
          <Label htmlFor="rt-prod">{u("perintahProduksiLabel")}</Label>
          <Select
            id="rt-prod"
            className="max-w-md"
            value={prodId}
    ...
    <Tr className="align-top">
      ...
      <Td label={u("status")}>
        <Badge tone={step.status === "done" ? "green" : "amber"}>
          {step.status === "done" ? u("statusSelesai") : "WIP"}
        </Badge>
      </Td>
      {isAdmin ? (
        <Td>
