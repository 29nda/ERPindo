...
export function CurrenciesPage() {
  ...
  //
  // Ditaruh di halaman ini, bukan di Pengaturan: kurs penutup yang dipakainya
  // adalah kurs yang persis terlihat di tabel bawah. Memisahkan keduanya berarti
  // pemilik menekan tombol tanpa melihat angka yang akan dipakainya.
  const [asOf, setAsOf] = useState(() => new Date().toISOString().slice(0, 10));
  const [revalError, setRevalError] = useState<string | null>(null);
  const [revalHasil, setRevalHasil] = useState<{
    entryNo: string;
  ...
  const revaluasi = useMutation({
    mutationFn: () => api.forexRevaluation(tenant.tenantId, asOf),
    onSuccess: (res) => {
      setRevalError(null);
      setRevalHasil(res);
      toast("success", u("revaluasiBerhasil"));
    },
    onError: (err) => {
      setRevalHasil(null);
      setRevalError((err as Error).message);
    },
  });
  ...
  return (
    <div className="max-w-3xl space-y-6">
      ...
      {isAdmin ? (
        <Card>
          <CardHeader title={u("revaluasiValas")} description={u("descRevaluasiValas")} />
          <CardBody className="space-y-4">
            {revalError ? <Alert tone="error">{revalError}</Alert> : null}
            {revalHasil ? (
              <Alert tone="success">
                <span data-testid="reval-hasil">
                  {revalHasil.jumlahDokumen} {u("revaluasiDokumen")} · {u("revaluasiSelisih")}:{" "}
                  Rp {revalHasil.labaBersih.toLocaleString("id-ID")} · {revalHasil.entryNo} ·{" "}
                  {u("revaluasiPembalik")}: {revalHasil.entryNoPembalik}
                </span>
              </Alert>
            ) : null}
            <div className="grid gap-3 sm:grid-cols-3">
              <div>
                <Label htmlFor="reval-asof">{u("tglAkhirPeriode")}</Label>
                <Input
                  id="reval-asof"
                  type="date"
                  value={asOf}
            ...
            <div className="flex justify-end">
              <Button variant="secondary" onClick={() => revaluasi.mutate()} disabled={revaluasi.isPending || !asOf}>
                {revaluasi.isPending ? <Spinner /> : null} {u("jalankanRevaluasi")}
              </Button>
            </div>
          </CardBody>
        </Card>
