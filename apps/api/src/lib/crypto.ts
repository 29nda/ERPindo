...
export async function verifyPassword(password: string, stored: string): Promise<boolean> {
  ...
  return samaAman(actual, expected);
}

/**
 * Bandingkan dua string ASCII **constant-time** — dipakai verifikasi password
 * dan token webhook masuk (`x-callback-token` Xendit, Fase 25a). Panjang yang
 * berbeda memang keluar lebih awal: panjangnya bukan rahasia, isinya yang rahasia.
 */
