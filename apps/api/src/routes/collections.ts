...
import { billingConfigured, buatInvoiceXendit } from "./billing";

/**
 * Payment collection (Fase 11d): buat link pembayaran online (invoice Xendit)
 ...
 *
 * Degradasi anggun: tanpa XENDIT_SECRET_KEY, pembuatan link membalas 503.
...
export const collectionRoutes = new Hono<AppEnv>()
  ...
  .post("/:tenantId/invoices/:id/payment-link", requireAuth, async (c) => {
    ...
    return c.json({ orderId, redirectUrl: bayar.redirectUrl, amount: outstanding }, 201);
  });
