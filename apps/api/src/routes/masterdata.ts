import { contactSchema, importRowsSchema, priceGroupSchema, productSchema, warehouseSchema, type CustomFieldModule } from "@erpindo/shared";
...
export const masterDataRoutes = new Hono<AppEnv>()
  ...
  .route(
    "/",
    crudRoutes("price-groups", {
      table: "price_groups",
      auditPrefix: "masterdata.priceGroup",
      schema: priceGroupSchema,
      uniqueField: { column: "name", input: "Nama grup" },
      searchColumns: ["name"],
      toRow: (g) => ({ name: g.name }),
    }),
  )
