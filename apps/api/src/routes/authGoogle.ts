...
import { buatState, pakaiState } from "../lib/oauthState";
...
/**
 * Cookie nonce alur login (Fase 26b). Alur ini dimulai TANPA sesi, jadi state
 * tidak bisa diikat ke `session_id` seperti alur Drive; ikatannya dipindah ke
 * cookie berumur pendek yang hanya dimiliki peramban pemulai. State di URL dan
 * nonce di cookie harus cocok — pola double-submit.
 */
const LOGIN_STATE_COOKIE = "erpindo_goauth";

/**
 * Payload id_token tanpa verifikasi tanda tangan.
 *
 * Ini DISENGAJA dan bukan kelalaian: token diambil langsung dari endpoint token
 * Google lewat TLS pada pertukaran server-ke-server dengan `client_secret` kita
 * sendiri. OpenID Connect Core §3.1.3.7 secara eksplisit mengizinkan validasi
 * TLS menggantikan pengecekan tanda tangan pada alur authorization-code seperti
 * ini. Yang TIDAK boleh dilewati adalah klaim `email_verified` — lihat
 * pemakaiannya di callback.
 */
function jwtIdentity(
  idToken: string | undefined,
): { sub: string; email: string; name: string | null; emailVerified: boolean } | null {
  try {
    ...
    return {
      ...
      // Google mengirimkannya sebagai boolean, tetapi beberapa penerbit
      // mengirim string "true" — keduanya diterima, apa pun selain itu tidak.
...
export const googleAuthRoutes = new Hono<AppEnv>()
  ...
  .get("/", rateLimit({ key: "google-auth", limit: 20, windowSeconds: 300 }), async (c) => {
    ...
    // Fase 26b (temuan audit B/I): state lama konstan untuk SEMUA orang dan
    // endpoint ini publik — siapa pun bisa memulai alur lalu membaca state
    // "sah" dari URL redirect. State yang bisa diambil siapa saja tidak
    // memberi perlindungan CSRF apa pun: penyerang tinggal menempelkan `code`
    // miliknya lalu membuat korban mengeklik callback, dan korban masuk ke
    // akun penyerang.
    const state = await buatState(c.env, { purpose: "login-google" });
    const appUrl = appOrigin(c.env, c.req.url);
  ...
  .get("/callback", rateLimit({ key: "google-cb", limit: 20, windowSeconds: 300 }), async (c) => {
    ...
    const nonceCookie = getCookie(c, LOGIN_STATE_COOKIE) ?? "";
    deleteCookie(c, LOGIN_STATE_COOKIE, { path: "/api/auth/google" });
    // Dua syarat, keduanya wajib: state harus cocok dengan nonce di cookie
    // peramban ini (double-submit), DAN state harus masih ada di database
    // (sekali pakai, maksimal 10 menit). Yang pertama mengikat alur ke peramban
    // pemulai; yang kedua mencegah pemakaian ulang.
    if (!code || !state || !nonceCookie || nonceCookie !== state) {
      return c.json({ error: "State OAuth tidak valid." }, 400);
    }
    const stateSah = await pakaiState(c.env, state, { purpose: "login-google" });
    if (!stateSah) return c.json({ error: "State OAuth tidak valid." }, 400);
    ...
    /**
     * Fase 26b (temuan audit J2): tolak email yang belum diverifikasi Google.
     *
     * Langkah (2) di bawah menautkan identitas Google ke akun password yang
     * SUDAH ADA berdasarkan alamat email. Tanpa pemeriksaan ini, siapa pun yang
     * bisa menerbitkan identitas Google beralamat email korban — akun Workspace
     * di domain yang ia kuasai, misalnya — mengambil alih akun ERPindo korban
     * hanya dengan menekan "Lanjutkan dengan Google". Ini jalur pengambilalihan
     * akun yang klasik, dan `sub` yang stabil tidak menolongnya karena
     * penautannya justru terjadi lewat email.
     */
    if (!identity.emailVerified) return back("email-google-belum-terverifikasi");
