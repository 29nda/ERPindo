...
/**
 * Akun yang dianggap KAS untuk laporan arus kas dan proyeksinya (Fase 22f).
 *
 * Satu definisi untuk keduanya, dan itu perbaikan — bukan kerapian. Sebelumnya
 * laporan arus kas memakai `1-1000, 1-1100` saja, sementara `1-1050 Kas Kecil`
 * yang ditambahkan Fase 22c tidak pernah dimasukkan. Akibatnya uang yang
 * dipindahkan ke kas kecil HILANG dari laporan arus kas — saldo akhirnya
 * terlalu kecil sebesar isi kotak, tanpa satu pun gerbang yang menangkapnya
 * karena tidak ada cek yang membandingkan kedua angka itu.
 *
 * Ketahuan lewat pemeriksaan mata Fase 22f: kartu proyeksi dan laporan arus kas
 * berdampingan di satu layar dan menyebut dua saldo kas yang berbeda.
 */
export const KODE_AKUN_KAS = ["1-1000", "1-1050", "1-1100"] as const;
const CASH_FILTER = `a.code IN (${KODE_AKUN_KAS.map((k) => `'${k}'`).join(", ")})`;
...
export const reportRoutes = new Hono<AppEnv>()
  ...
  // -------------------------------------------------------------------------
  .get("/:tenantId/reports/cash-projection", requireAuth, requireTenantRole("viewer"), heavyLimit(), async (c) => {
    const db = getTenantDb(c.env, c.get("tenant").dbRef);
    const hariIni = new Date().toISOString().slice(0, 10);
    const batasAkhir = new Date(Date.parse(`${hariIni}T00:00:00Z`) + 90 * 86_400_000).toISOString().slice(0, 10);

    // Saldo kas: akun kas/bank sistem saja. Sengaja TIDAK memakai heuristik nama
    // akun seperti halaman Kas & Bank — akun bernama "Bank Garansi" (yang bukan
    // kas) akan ikut terhitung dan menggeser seluruh proyeksinya.
    // Saldo awal DIBATASI sampai hari ini. Tanpa batas itu, jurnal bertanggal
    // masa depan (mis. faktur kontrak yang sudah diposting di muka) terhitung
    // DUA KALI: sekali di saldo awal, sekali lagi sebagai arus yang
    // diproyeksikan.
    const saldo = (
      await db
        .prepare(
          `SELECT COALESCE(SUM(l.debit - l.credit), 0) AS s
           ...
           JOIN accounts a ON a.id = l.account_id
           WHERE e.status = 'posted' AND ${CASH_FILTER} AND e.entry_date <= ?`,
        )
        .bind(hariIni)
        .all<{ s: number }>()
    ).results[0]?.s ?? 0;

    const [piutang, hutang, kontrak] = await Promise.all([
      db
        .prepare(
          `SELECT d.invoice_no AS no, k.name AS nama, COALESCE(d.due_date, d.invoice_date) AS jatuh,
                  d.total - d.paid_amount - d.returned_amount AS sisa
           ...
           WHERE d.status != 'paid' AND d.voided_at IS NULL
             ...
             AND COALESCE(d.due_date, d.invoice_date) <= ?`,
        )
        .bind(batasAkhir)
        .all<{ no: string; nama: string; jatuh: string; sisa: number }>(),
      db
        .prepare(
          `SELECT d.purchase_no AS no, k.name AS nama, COALESCE(d.due_date, d.purchase_date) AS jatuh,
                  d.total - d.paid_amount - d.returned_amount AS sisa
           FROM purchases d JOIN contacts k ON k.id = d.contact_id
           WHERE d.status != 'paid' AND d.voided_at IS NULL
             AND d.total > d.paid_amount + d.returned_amount
             AND COALESCE(d.due_date, d.purchase_date) <= ?`,
        )
        .bind(batasAkhir)
        .all<{ no: string; nama: string; jatuh: string; sisa: number }>(),
      // Kontrak berulang: hanya faktur BERIKUTNYA yang diproyeksikan, bukan
      // seluruh sisa masa kontrak. Memproyeksikan semuanya berarti mengarang
      // pendapatan berbulan-bulan ke depan dari kontrak yang bisa dihentikan
      // kapan saja — angka besar yang terasa meyakinkan justru karena besarnya.
      db
        .prepare(
          `SELECT ct.code AS no, k.name AS nama, ct.next_invoice_date AS jatuh,
                  COALESCE((SELECT SUM(cl.qty * cl.unit_price) FROM contract_lines cl WHERE cl.contract_id = ct.id), 0) AS sisa
           FROM contracts ct JOIN contacts k ON k.id = ct.contact_id
           WHERE ct.status = 'active' AND ct.next_invoice_date <= ?`,
        )
        .bind(batasAkhir)
        .all<{ no: string; nama: string; jatuh: string; sisa: number }>(),
    ]);

    const arus: ArusDiproyeksikan[] = [
      ...piutang.results.map((r) => ({
        tanggal: r.jatuh,
        jumlah: r.sisa,
        sumber: "piutang" as const,
        keterangan: `${r.no} — ${r.nama}`,
        terlambat: r.jatuh < hariIni,
      })),
      ...hutang.results.map((r) => ({
        tanggal: r.jatuh,
        jumlah: -r.sisa,
        sumber: "hutang" as const,
        keterangan: `${r.no} — ${r.nama}`,
        terlambat: r.jatuh < hariIni,
      })),
      ...kontrak.results
    ...
    return c.json({ hariIni, ...proyeksikanArusKas(saldo, arus, hariIni), arus });
  })
  ...
  .get("/:tenantId/dashboard", requireAuth, requireTenantRole("viewer"), async (c) => {
    ...
    const [cashRows, salesRows, lastMonthRows, arRows, apRows, stockRows, leadRows, plCur, plPrev, plLastYear, lastYearSalesRows] = await Promise.all([
      db
        .prepare(
          `SELECT COALESCE(SUM(l.debit - l.credit), 0) AS balance
           FROM journal_lines l
      ...
      // Bulan yang sama setahun lalu (Fase 21e). `monthStart` menghitung lewat
      // `getUTCMonth() + offset`, jadi lompatan tahunnya ditangani Date sendiri
      // — tidak ada aritmetika tahun buatan tangan yang bisa salah di Januari.
      profitLoss(db, monthStart(-12), monthStart(-11)),
      db
        .prepare(`SELECT COALESCE(SUM(total), 0) AS total FROM invoices WHERE voided_at IS NULL AND invoice_date LIKE ?`)
        .bind(`${monthStart(-12).slice(0, 7)}%`)
        .all<{ total: number }>(),
    ]);
    ...
    const body: ApiDashboard = {
      ...
      salesLastYear: lastYearSalesRows.results[0]?.total ?? 0,
      profitLastYear: plLastYear.profit,
    };
    return c.json(body);
  });
