...
export const taxRoutes = new Hono<AppEnv>()
  ...
  /**
   * Kalender pajak (Fase 22e) — BACA-SAJA.
   *
   * Hanya kewajiban yang benar-benar berlaku bagi tenant yang dikembalikan.
   * Kalender yang memuat kewajiban asing (PPN untuk non-PKP, PPh 21 untuk usaha
   * tanpa karyawan) melatih orang mengabaikannya, dan pengingat yang diabaikan
   * sama saja tidak ada.
   */
  .get("/:tenantId/tax/calendar", requireAuth, requireTenantRole("viewer"), requirePermission("pajak"), async (c) => {
    const db = getTenantDb(c.env, c.get("tenant").dbRef);
    ...
    const profil = {
      pkp: await setting("pkp"),
      pphFinalUmkm: await setting("pph_final_umkm"),
      badan: await setting("badan_usaha"),
      // Dibaca dari data, bukan ditanyakan — satu-satunya medan profil yang
      // bisa disimpulkan dengan aman.
      adaKaryawan: jml > 0,
    };

    const acuan = new Date(`${hariIni}T00:00:00Z`);
    const masaList = [0, 1, 2, 3].map((mundur) =>
      new Date(Date.UTC(acuan.getUTCFullYear(), acuan.getUTCMonth() - mundur, 1)).toISOString().slice(0, 7),
    );
    /**
     * Jendelanya sengaja SEMPIT (−30 s/d +60 hari), dan urutannya menaruh yang
     * BELUM lewat lebih dulu.
     *
     * Pemeriksaan mata Fase 22e menemukan versi pertama membuka daftar dengan
     * 14 baris "Terlambat" berturut-turut — perusahaan yang belum pernah
     * melapor otomatis menumpuk seluruh riwayatnya di puncak layar. Daftar
     * pengingat yang dibuka dengan tembok tunggakan justru melatih orang
     * mengabaikannya, yaitu persis kerusakan yang fitur ini ada untuk mencegah.
     *
     * Yang sudah lewat TIDAK disembunyikan — dendanya masih bisa diperkecil
     * dengan segera menyetor — hanya dipindah ke bawah, sesudah yang masih
     * bisa dikerjakan tepat waktu.
     */
    const semua = [
      ...masaList.flatMap((m) => tenggatMasaPajak(m, profil, hariIni)),
      tenggatSptTahunan(acuan.getUTCFullYear() - 1, profil, hariIni),
    ].filter((t) => t.sisaHari >= -30 && t.sisaHari <= 60);

    const urut = (a: TenggatPajak, b: TenggatPajak) => a.sisaHari - b.sisaHari || a.jenis.localeCompare(b.jenis);
    const tenggat: TenggatPajak[] = [
      ...semua.filter((t) => t.sisaHari >= 0).sort(urut),
      // Yang terlambat: paling BARU terlewat lebih dulu — itu yang dendanya
      // masih paling kecil dan paling masuk akal dikejar hari ini.
      ...semua.filter((t) => t.sisaHari < 0).sort((a, b) => b.sisaHari - a.sisaHari || a.jenis.localeCompare(b.jenis)),
    ];

    return c.json({ hariIni, profil, tenggat });
  });
