...
export const driveRoutes = new Hono<AppEnv>()
  ...
  .get("/:tenantId/drive/connect", requireAuth, requireTenantRole("owner"), async (c) => {
    ...
    const params = new URLSearchParams({
      ...
      access_type: "offline",
      prompt: "consent",
      // Fase 26b: state acak sekali-pakai yang terikat SESI pemulainya, bukan
      // lagi turunan tetap dari tenantId + rahasia klien.
      state: await buatState(c.env, {
        purpose: "drive",
        tenantId: tenant.id,
        sessionId: c.get("user").sessionId,
      }),
    });
...
export const driveCallbackRoutes = new Hono<AppEnv>().get("/callback", requireAuth, async (c) => {
  ...
  if (!code) return back("state-tidak-valid");

  // Fase 26b (temuan audit B): state ditukar di sini — sekali pakai, berumur
  // 10 menit, dan hanya sah bila SESI yang menukarnya adalah sesi yang
  // menerbitkannya. Tanpa ikatan itu, callback yang dibuat orang lain (berisi
  // `code` untuk akun Google MILIK DIA) bisa diselesaikan oleh peramban korban
  // — dan refresh token penyerang yang tersimpan sebagai integrasi tenant
  // korban. Cookie sesi ber-SameSite=Lax tetap terkirim pada navigasi GET
  // tingkat-atas, jadi "korban mengeklik tautan" sudah cukup.
  const terpakai = await pakaiState(c.env, state, { purpose: "drive", sessionId: c.get("user").sessionId });
  if (!terpakai?.tenantId) return back("state-tidak-valid");
  const tenantId = terpakai.tenantId;
