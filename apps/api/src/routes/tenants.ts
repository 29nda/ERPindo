...
export const tenantRoutes = new Hono<AppEnv>()
  ...
  .post("/:tenantId/invites", requireAuth, requireTenantRole("admin"), async (c) => {
    ...
    if ((memberCount?.n ?? 0) >= PLAN_LIMITS[plan].maxUsers) {
      return c.json(
        {
          error: `Paket ${PLAN_LABELS[plan]} maksimal ${PLAN_LIMITS[plan].maxUsers} pengguna.`,
        },
        402,
      );
    }
  ...
  .patch("/:tenantId/roles/:roleId", requireAuth, requireTenantRole("owner"), async (c) => {
    ...
    // jarak dua pernyataan; membawa `tenant_id` ke UPDATE menjadikannya sifat
    // pernyataannya sendiri. Tidak ada eksploit yang diketahui hari ini — yang
    // dijaga adalah invariannya, supaya penulisan ulang berikutnya tidak
    // kehilangannya tanpa terlihat.
    await c.env.DB.prepare(
      `UPDATE custom_roles SET name = ?, base_role = ?, permissions = ?, scope_cost_center_ids = ?
       WHERE id = ? AND tenant_id = ?`,
    )
      .bind(parsed.data.name, parsed.data.baseRole, JSON.stringify(parsed.data.permissions), scopeJsonUpd, roleId, tenant.id)
      .run();
    // Sinkronkan base_role ke anggota yang memakai peran ini (kompat requireTenantRole).
    await c.env.DB.prepare(`UPDATE memberships SET role = ? WHERE custom_role_id = ? AND tenant_id = ?`)
      .bind(parsed.data.baseRole, roleId, tenant.id)
      .run();
  ...
  .delete("/:tenantId/roles/:roleId", requireAuth, requireTenantRole("owner"), async (c) => {
    const tenant = c.get("tenant");
    const roleId = c.req.param("roleId");
    const used = await c.env.DB.prepare(`SELECT COUNT(*) AS n FROM memberships WHERE custom_role_id = ? AND tenant_id = ?`)
      .bind(roleId, tenant.id)
      .first<{ n: number }>();
  ...
  .patch("/:tenantId/settings", requireAuth, requireTenantRole("admin"), async (c) => {
    ...
    const entries = Object.entries({
      ...
      pkp: parsed.data.pkp === undefined ? undefined : parsed.data.pkp ? "1" : "0",
      pph_final_umkm: parsed.data.pphFinalUmkm === undefined ? undefined : parsed.data.pphFinalUmkm ? "1" : "0",
      badan_usaha: parsed.data.badanUsaha === undefined ? undefined : parsed.data.badanUsaha ? "1" : "0",
  ...
  .get("/:tenantId/notifications", requireAuth, requireTenantRole("viewer"), async (c) => {
    ...
    // penyaringannya: hanya kewajiban yang benar-benar berlaku bagi tenant ini.
    const profil = {
      pkp: (await bacaSetting(db, "pkp")) === "1",
      pphFinalUmkm: (await bacaSetting(db, "pph_final_umkm")) === "1",
      badanUsaha: (await bacaSetting(db, "badan_usaha")) === "1",
    };
    // "Punya karyawan" dibaca dari data, bukan ditanyakan — satu-satunya dari
    // keempat medan profil yang bisa disimpulkan dengan aman.
    const jmlKaryawan = (
      await db.prepare(`SELECT COUNT(*) AS n FROM employees WHERE is_active = 1`).all<{ n: number }>()
    ).results[0]?.n ?? 0;
    const profilPajak = { ...profil, badan: profil.badanUsaha, adaKaryawan: jmlKaryawan > 0 };

    // Masa pajak yang tenggatnya bisa jatuh di jendela pengingat: masa bulan
    // lalu (tenggatnya bulan ini) dan masa dua bulan lalu (yang baru terlewat).
    const acuan = new Date(`${today}T00:00:00Z`);
    const masaList = [1, 2].map((mundur) => {
      const d = new Date(Date.UTC(acuan.getUTCFullYear(), acuan.getUTCMonth() - mundur, 1));
      return d.toISOString().slice(0, 7);
    });
    const semuaTenggat = [
      ...masaList.flatMap((m) => tenggatMasaPajak(m, profilPajak, today)),
