...
const ORDER_SELECT = `SELECT o.id, o.order_no, o.product_id, p.name AS product_name,
    ...
    o.unit_cost, o.total_cost, o.labour_cost, o.overhead_cost, qw.name AS qc_warehouse_name, o.created_at, o.produced_at
...
export const manufacturingRoutes = new Hono<AppEnv>()
  ...
  .post("/:tenantId/production-orders", requireAuth, requireTenantRole("admin"), async (c) => {
    ...
    await db
      .prepare(
        `INSERT INTO production_orders (id, order_no, product_id, warehouse_id, qty, status, qc_status,
                                        labour_cost, overhead_cost, created_by)
         VALUES (?, ?, ?, ?, ?, 'draft', 'none', ?, ?, ?)`,
      )
      .bind(
        id, orderNo, input.productId, input.warehouseId, input.qty,
        input.labourCost, input.overheadCost, c.get("user").id,
      )
      .run();
  ...
  .post("/:tenantId/production-orders/:id/complete", requireAuth, requireTenantRole("admin"), async (c) => {
    ...
    const { results: orderRows } = await db
      .prepare(
        `SELECT id, product_id, warehouse_id, qty, status, labour_cost, overhead_cost
    ...
    // Biaya konversi (tenaga kerja + overhead) ikut masuk nilai barang jadi —
    // Fase 21f. Rumusnya dipakai bersama kalkulator HPP di layar Alat Bantu.
    const biaya = hitungBiayaProduksi({
      biayaBahan: totalCost,
      biayaTenagaKerja: order.labour_cost,
      biayaOverhead: order.overhead_cost,
      qty: order.qty,
    });

    // Hasil produk jadi dengan biaya rata-rata gabungan.
    const unitCost = biaya.biayaPerUnit;
    await stockIn(db, {
    ...
    /*
     * JURNAL PENYERAPAN — bagian yang paling mudah dilupakan, dan kelalaiannya
     * TIDAK tertangkap neraca saldo.
     *
     * Produksi tanpa biaya konversi memang tak butuh jurnal: bahan keluar
     * senilai X, barang jadi masuk senilai X, jadi total nilai persediaan tidak
     * berubah dan buku besar tetap cocok dengan `stock_levels`.
     *
     * Begitu tenaga kerja & overhead ikut dikapitalisasi, nilai `stock_levels`
     * NAIK. Tanpa jurnal ini, buku besar Persediaan tertinggal dan kedua angka
     * itu berpisah diam-diam — sementara neraca saldo tetap seimbang, karena
     * memang tidak ada jurnal yang dibuat untuk diperiksa.
     *
     * Sisi kreditnya kontra-beban, bukan kas: gaji & listriknya SUDAH dibayar
     * dan sudah dibebankan. Mengkreditnya di sini membatalkan beban itu
     * sebatas yang terserap; selisihnya tetap tertinggal di laba rugi sebagai
     * selisih serapan, dan memang di situlah tempatnya.
     */
    if (biaya.biayaDiserap > 0) {
      const [persediaan, diserap] = await Promise.all([
        accountIdByCode(db, SYS_ACCOUNTS.PERSEDIAAN),
        accountIdByCode(db, SYS_ACCOUNTS.PRODUKSI_DISERAP),
      ]);
      await postJournal(db, {
        entryDate: new Date().toISOString().slice(0, 10),
        memo: `Penyerapan biaya produksi ${order.id.slice(0, 8)} — tenaga kerja & overhead`,
        createdBy: c.get("user").id,
        lines: [
          { accountId: persediaan, description: "Biaya konversi", debit: biaya.biayaDiserap, credit: 0 },
          { accountId: diserap, description: "Biaya konversi", debit: 0, credit: biaya.biayaDiserap },
        ],
      });
    }
    ...
    await db
      ...
      .bind(unitCost, biaya.totalBiaya, orderId)
      .run();
    ...
    await audit(c.env, {
      ...
      userId: c.get("user").id,
      tenantId: tenant.id,
      detail: { orderId, qty: order.qty, totalCost: biaya.totalBiaya, biayaDiserap: biaya.biayaDiserap, unitCost },
      ip: clientIp(c),
    });
    return c.json({ ok: true, qty: order.qty, unitCost, totalCost: biaya.totalBiaya, biayaDiserap: biaya.biayaDiserap }, 200);
  })
