...
export const crmRoutes = new Hono<AppEnv>()
  ...
  // -------------------------------------------------------------------------
  // Form lead publik (Fase 21e) — token yang ditanam pemilik di form miliknya.
  // -------------------------------------------------------------------------

  /** Token & slug yang dibutuhkan layar untuk menyusun cuplikan formnya. */
  .get("/:tenantId/lead-form", requireAuth, requireTenantRole("owner"), async (c) => {
    const tenant = c.get("tenant");
    const db = getTenantDb(c.env, tenant.dbRef);
    const { results } = await db
      .prepare(`SELECT value FROM settings WHERE key = 'lead_form_token'`)
      .all<{ value: string }>();
    return c.json({ token: results[0]?.value ?? null, slug: tenant.slug });
  })

  /**
   * Terbitkan / putar ulang token. Dipakai juga sebagai tombol "putar ulang"
   * bila formnya disalahgunakan — tokennya memang tertanam di HTML publik,
   * jadi memutarnya harus semudah menekan satu tombol.
   */
  .post("/:tenantId/lead-form/token", requireAuth, requireTenantRole("owner"), async (c) => {
    const tenant = c.get("tenant");
    const db = getTenantDb(c.env, tenant.dbRef);
    const token = generateToken();
    await db
      .prepare(
        `INSERT INTO settings (key, value, updated_at) VALUES ('lead_form_token', ?, ?)
         ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at`,
      )
      .bind(token, new Date().toISOString())
      .run();
    await audit(c.env, {
      action: "crm.lead_form_token_rotated",
      userId: c.get("user").id,
      tenantId: tenant.id,
      detail: {},
      ip: clientIp(c),
  ...
  .delete("/:tenantId/lead-form/token", requireAuth, requireTenantRole("owner"), async (c) => {
    const db = getTenantDb(c.env, c.get("tenant").dbRef);
    await db.prepare(`DELETE FROM settings WHERE key = 'lead_form_token'`).run();
    return c.json({ ok: true });
  });
