...
import { leadFormRoutes } from "./routes/leadForm";
...
import { priceGroupRoutes } from "./routes/priceGroups";
...
import { enforceTenantAccessByPath } from "./middleware/auth";
...
const app = new Hono<AppEnv>()
  .use(
    ...
    secureHeaders({
      ...
      contentSecurityPolicy: {
        defaultSrc: ["'self'"],
        // `wasm-unsafe-eval` ditambahkan Fase 21g. Namanya menakutkan tetapi
        // izinnya sempit: ia HANYA membolehkan kompilasi WebAssembly, dan
        // tidak menghidupkan kembali `eval()` maupun `new Function()` untuk
        // JavaScript. Tanpa ini pengurai barcode cadangan (Safari iOS) ditolak
        // peramban sebelum satu byte pun dijalankan.
      ...
      // `camera: ["self"]` → `camera=(self)`, BUKAN `camera=()`.
      //
      // Sampai Fase 21g nilainya `[]`, yang diterjemahkan jadi `camera=()` —
      // artinya kamera ditutup untuk SEMUA ORANG, termasuk aplikasi ini
      // sendiri. Pemindai barcode POS yang dibangun Fase 20i karena itu tidak
      // pernah bisa berjalan di produksi: `getUserMedia()` ditolak peramban
      // sebelum dialog izin muncul, dan kasir hanya melihat pesan "izin
      // ditolak" yang menyesatkan. Tidak ada gerbang yang menangkapnya karena
      // Chromium ui-sim memang tak punya kamera sama sekali.
  ...
  .use("/api/tenants/:tenantId/*", enforceTenantAccessByPath)
  ...
  .route("/api/tenants", priceGroupRoutes)
  ...
  // Form lead publik per tenant (Fase 21e) — tanpa sesi, dibatasi laju per IP.
  .route("/api/form", leadFormRoutes)
  .route("/", blogRoutes)
...
async function scheduled(_event: ScheduledEvent, env: Env, _ctx: ExecutionContext): Promise<void> {
  ...
  // Fase 24: blok "trial berakhir" DIHAPUS bersama trial itu sendiri. Tenant
  // yang belum berlangganan berstatus `provisioning` dan tidak punya tanggal
  // apa pun untuk kedaluwarsa — ia menunggu pembayaran, bukan menunggu waktu.

  // 1b) Langganan berbayar habis (Fase 11b): active → past_due saat
  //     subscription_ends_at lewat. Comped (subscription_ends_at NULL) tak
  //     tersentuh. Bayar via Xendit mengembalikan ke 'active' lewat webhook.
  ...
  // 1b2) PENCABUTAN penurunan paket terjadwal (Fase 30).
  //
  //      Blok ini dulu menerapkan `pending_plan` saat periode berbayar habis.
  //      Dengan satu paket tidak ada penurunan yang bisa dijadwalkan; endpoint
  //      yang mengisi kolom itu sudah dicabut, dan migrasi 0017 mengosongkan
  //      sisa nilainya. Membiarkan kueri ini berjalan tiap hari hanya menambah
  //      satu pembacaan D1 untuk hasil yang dijamin kosong selamanya.
  ...
  //      masih benar. Sekali per tenant.
  {
    ...
    const { results: tenggang } = await env.DB.prepare(
      `SELECT id, name, status FROM tenants
        WHERE (status = 'active' AND subscription_ends_at IS NOT NULL AND subscription_ends_at > ? AND subscription_ends_at <= ?)
  ...
  //
  //    Sejak Fase 24 hanya pelanggan berbayar yang tersisa untuk diperingatkan
  //    (trial dihapus). Dua tonggak (H-7 dan H-1), masing-masing dikirim sekali
  //    per tenant dan ditandai di KV — tonggak dipisah supaya pengingat H-1
  //    tetap terkirim walau H-7 sudah lewat.
  ...
  //     spam, yang justru merugikan pengingat H-7/H-1 yang benar-benar penting.
  {
    ...
    const { results: tertunggak } = await env.DB.prepare(
      // Sengaja TIDAK mengambil tanggal jatuhnya — emailnya tak memerlukannya.
      // Sejak Fase 24 hanya `subscription_ends_at` yang relevan; `trial_ends_at`
...
// Diekspor agar `test/rbac-registry.test.ts` bisa membaca daftar route terpasang
// yang SEBENARNYA (`app.routes`) — termasuk route yang dibangun dari template
// dan tidak akan tertangkap pembacaan berkas. Lihat uji itu untuk alasannya.
export { app };
