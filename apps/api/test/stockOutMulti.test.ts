          ...
          return {
            ...
            async run() {
              ...
              if (q.startsWith("UPDATE stock_levels SET qty = qty - ?")) {
                // Fase 29a: pengurangannya kini BERSYARAT (`AND qty >= ?`) dan
                // `stockOut` memutuskan menang/kalah dari `meta.changes`. Fixture
                // ini ikut menirukan keduanya — kalau ia terus mengembalikan
                // `{}` polos, penjaga baru akan membaca "0 baris berubah" dan
                // menolak setiap pengambilan yang sebenarnya sah.
                const lv = levels[String(v[2])];
                const diminta = Number(v[0]);
                if (lv && lv.qty >= diminta) {
                  lv.qty -= diminta;
