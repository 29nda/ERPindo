...
const MANIFESTS = {
  ...
  panduan: {
    ...
    shots: [
      ...
      // Fase 25b: dua modul yang di /fitur terpaksa dirender TANPA gambar sejak
      // Fase 24c. Kas & Bank sekadar terlewat dari manifest; Asisten AI tidak
      // punya rute sendiri, jadi tombol mengambangnya diklik dulu.
      { route: "/app/keuangan/kas-bank", name: "kasbank-1", width: 1280, waitMs: 1200 },
      {
        route: "/app",
        name: "asisten-1",
        width: 1280,
        waitMs: 1500,
        klik: '[aria-label="Buka Asisten erpindo"]',
        // Lingkungan tangkapan layar tidak punya binding Workers AI, sehingga
        // widget ringkasan mingguan menampilkan "Fitur AI belum tersedia" —
        // benar untuk dev, KELIRU untuk produksi (Asisten terverifikasi
        // menjawab di sana sejak Fase 5a). Membiarkannya berarti gambar modul
        // Asisten AI di halaman jualan mengumumkan bahwa fiturnya mati.
        // Dihapus dengan mekanisme yang sama seperti banner "belum
        // diverifikasi" di bawah: artefak lingkungan, bukan perilaku produk.
        sembunyikanTeks: "Fitur AI belum tersedia",
      },
    ],
  },
...
try {
  ...
  for (const vp of viewports) {
    ...
    for (const shot of manifest.shots) {
      ...
      // Fase 25b: sebagian permukaan bukan halaman tersendiri — Asisten AI
      // adalah panel mengambang yang hanya ada setelah tombolnya ditekan.
      // Tanpa ini modul itu mustahil punya tangkapan layarnya sendiri, dan di
      // Fase 24c ia terpaksa dirender tanpa gambar di halaman /fitur.
      if (shot.klik) {
        await page.click(shot.klik);
        await page.waitForTimeout(shot.klikTungguMs ?? 700);
      }
      ...
      // Elemen yang hanya muncul karena keterbatasan lingkungan tangkapan layar
      // (lihat `sembunyikanTeks` di manifest) — bukan perilaku produk.
      if (shot.sembunyikanTeks) {
