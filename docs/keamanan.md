...
## Kebijakan tujuan webhook keluar (SSRF)

Ditambahkan Fase 26d setelah audit keamanan. Sebelumnya URL webhook hanya
divalidasi bentuknya (`z.string().url()`), yang meloloskan `http://localhost`,
`http://10.0.0.1`, dan `http://169.254.169.254`.

**Yang ditolak** — di pintu masuk (400 saat dibuat/diubah) **dan** diperiksa
ulang tepat sebelum `fetch`, karena baris yang tersimpan sebelum kebijakan ini
ada tidak pernah melewati skemanya:

- skema selain `https://`;
- kredensial di dalam URL (`https://pengguna:sandi@…`);
- `localhost`, `*.localhost`, `*.local`;
- IPv4 loopback/privat/link-local/CGNAT/multicast (`127/8`, `10/8`, `172.16/12`,
  `192.168/16`, `169.254/16`, `100.64/10`, `0/8`, `≥224`);
- IPv6 loopback/unspecified/link-local/unique-local, **termasuk bentuk
  IPv4-mapped yang sudah dinormalkan** (`::ffff:7f00:1`, bukan hanya
  `::ffff:127.0.0.1`);
- redirect **tidak diikuti** (`redirect: "manual"`) — tujuan publik yang sah bisa
  membalas 302 ke alamat internal, dan pemeriksaan hanya melihat URL pertama.
...
- **DNS rebinding tidak dicegah.** Nama yang sah saat disimpan bisa menunjuk
  ...
  hubungi IP yang sama), yang tidak tersedia di Cloudflare Workers.
- **Dampak SSRF di runtime ini memang terbatas** sejak awal: Workers tidak punya
  jaringan internal maupun endpoint metadata instans, jadi pivot ala VM tidak
  berlaku. Yang benar-benar ditutup kebijakan ini adalah kebocoran payload lewat
  `http://` polos, kredensial yang mengendap di database, dan tujuan internal
  yang tersimpan sebagai konfigurasi.
...
Setiap perubahan wajib lolos gerbang penuh (per 15 Agustus 2026: **1.112 skenario
smoke** end-to-end, **337 cek simulasi UI** browser nyata, **549 unit test**,
typecheck & lint) — termasuk uji kelengkapan registri akses rute, matriks izin
peran kustom, uji balapan token & webhook pembayaran, dan verifikasi neraca saldo
seimbang di setiap pembalikan transaksi.

> Angka lama di dokumen ini (749 / 160 / 33) sudah jauh tertinggal dan diperbarui
> pada Fase 26d. Angka gerbang cenderung basi diam-diam; yang berlaku selalu
> keluaran `pnpm test`, `pnpm smoke`, dan `node scripts/ui-sim.mjs` hari itu.
