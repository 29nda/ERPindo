...
| ✅ **Perbandingan periode otomatis** (bulan ini vs bulan lalu vs tahun lalu) pada KPI Penjualan & Laba — delta bulan lalu (Fase 12d) + `salesLastYear`/`profitLastYear` di `routes/reports.ts` lewat `monthStart(-12)` (Fase 21e). Delta tahun lalu **tidak tampil** bila bulan yang sama tahun lalu bernilai 0 — persentase dari pembagi nol tidak bermakna | T | R | – |
...
| Ide | Dampak | Usaha | AI |
|---|:---:|:---:|:---:|
| ✅ **Barcode scanner via kamera HP** — scan langsung menambah item ke keranjang (Fase 20i + cadangan wasm Fase 21g: `apps/web/src/lib/barcode.ts` memuat `barcode-detector`/`zxing-wasm` lewat `import()` dinamis hanya bila API bawaan absen, sehingga Safari iOS ikut terlayani; ui-sim `F38*` memindai barcode sungguhan lewat kamera palsu Chromium) | T | S | – |
...
| ✅ **Harga bertingkat per pelanggan/grup (harga grosir vs ecer)** — Fase 23a + 23b, `routes/priceGroups.ts` + `hargaUntukGrup()` + `lib/hargaGrup.ts`; dipakai di faktur, kasir (pemilih grup), pesanan penjualan, penawaran & kontrak. Disarankan, tidak ditegakkan | S | S | – |
...
| ✅ **Kas kecil (petty cash) dengan pengisian ulang berjurnal** — Fase 22c: akun sistem `1-1050 Kas Kecil` & `5-4900 Selisih Kas` (migrasi `0044`), empat endpoint di `apps/api/src/routes/financeExtras.ts` (status, dana tetap, isi ulang, opname). **Jumlah pengisian dihitung server** dari selisih dana tetap terhadap saldo buku besar — tidak pernah diterima dari klien. Bon dijurnal saat dicatat (bukan ditahan sampai pengisian), sehingga pengisian ulang adalah pemindahan murni yang **tidak menambah beban** dan hitung-ganda mustahil secara konstruksi. Opname membandingkan hitungan fisik dengan saldo buku dan menjurnal selisihnya ke `5-4900` — dua arah, satu akun. Kartu UI di `apps/web/src/pages/kasbank.tsx`; fungsi murni `hitungPengisianKasKecil()` & `hitungSelisihKas()` di `packages/shared/src/accounting.ts` | S | R | – |
| ✅ **Proyeksi arus kas 30–90 hari dari piutang/hutang jatuh tempo + kontrak berulang** — Fase 22f: `proyeksikanArusKas()` di `packages/shared/src/accounting.ts`, `GET /reports/cash-projection` **baca-saja** di `apps/api/src/routes/reports.ts`, kartu di halaman Arus Kas (`apps/web/src/pages/reports.tsx`). Tagihan yang **sudah lewat tempo tetap dihitung** (masuk ember pertama) dan jumlahnya dilaporkan terpisah; peringatan bila kas diperkirakan minus. Hanya **satu** faktur kontrak berikutnya yang diproyeksikan, bukan seluruh sisa masa kontrak. Asumsinya dinyatakan di layar: setiap tagihan dibayar tepat jatuh tempo | T | S | – |
...
| ✅ **Metode penyusutan saldo menurun (pilihan per aset)** — Fase 22d: `penyusutanBulanan()` di `packages/shared/src/hr.ts`, kolom `depreciation_method` (migrasi `0045`), dipakai `runDepreciation()` di `apps/api/src/routes/assets.ts`. Tarif 2/umur atas **nilai buku berjalan**, dengan **aturan penutup**: pada bulan terakhir masa manfaat seluruh sisa disusutkan sekaligus — tanpa itu rumusnya asimtotik dan asetnya tak pernah selesai. Garis lurus sengaja TIDAK memakai aturan kalender itu supaya angka aset lama tidak bergeser | S | S | – |
| Foto & lokasi aset + QR label untuk stock opname aset | R | S | – |
| ✅ **Penyusutan fiskal vs komersial berdampingan (kelompok harta pajak)** — Fase 22d: `KELOMPOK_HARTA` (UU PPh Ps. 11, 4/8/16/20 tahun + bangunan) & `ringkasPenyusutanFiskal()` di `packages/shared/src/hr.ts`; `GET /assets/tax-depreciation` **baca-saja** di `apps/api/src/routes/assets.ts`, kartu di `apps/web/src/pages/assets.tsx`. Angka fiskal **tidak pernah dijurnal** — buku besar tetap memuat penyusutan komersial, dan selisihnya disajikan sebagai koreksi fiskal untuk SPT. Bangunan ditolak memakai saldo menurun (Ps. 11 ayat 6), ditegakkan di skema maupun rute | S | T | – |
...
| Ide | Dampak | Usaha | AI |
|---|:---:|:---:|:---:|
| 🟡 **Tarik kurs referensi otomatis harian via Cron** — Fase 22b: `segarkanKursReferensi()` di `apps/api/src/routes/currencies.ts` dipanggil loop cron harian; `bacaKursReferensi()` (`packages/shared/src/accounting.ts`) menolak payload cacat, hanya menyegarkan mata uang yang **sudah terdaftar** tenant, dan kegagalan sumber **tidak menyentuh kurs lama**. Kolom "Terakhir diperbarui" di halaman Mata Uang. **Sebagian**: sumbernya masih generik (`{base,rates}` lewat `KURS_SOURCE_URL`) — **kurs pajak Kemenkeu belum dipakai**, bentuknya berbeda dan butuh pengurai tersendiri; jalur `fetch()` sungguhannya juga belum diuji gerbang (smoke memakai payload override) | S | S | – |
| ✅ **Revaluasi saldo valas akhir periode (unrealized gain/loss)** — Fase 22a: `postForexRevaluation()` di `apps/api/src/routes/financeExtras.ts` menilai ulang sisa piutang/hutang valas ke kurs penutup `currencies.rate`, lalu **membalik jurnalnya otomatis pada H+1** supaya subledger faktur tetap satu-satunya sumber AR/AP dan selisih kursnya tidak terhitung dua kali saat pelunasan. Tombolnya di halaman Mata Uang (`pages/currencies.tsx`) | S | S | – |
...
| Ide | Dampak | Usaha | AI |
|---|:---:|:---:|:---:|
| ✅ **Biaya overhead & tenaga kerja ke dalam HPP produksi** — `hitungBiayaProduksi()` di `packages/shared/src/ops.ts` dipakai perintah produksi `routes/manufacturing.ts`; biayanya dikapitalisasi ke persediaan lewat jurnal penyerapan ke akun baru `5-2100 Beban Produksi Diserap` (Fase 21f), sehingga upah & listrik tidak terhitung dua kali. Selisih serapan tetap terlihat di Laba Rugi | S | S | – |
...
akses penuh; pelanggan lama Rp389rb di-**grandfather** (`legacy_full_access`). Checkout Xendit
...
| Ide | Dampak | Usaha | AI |
|---|:---:|:---:|:---:|
| **Payment gateway Xendit** — checkout QRIS/VA/e-wallet aktif (**menunggu `XENDIT_SECRET_KEY` + `XENDIT_CALLBACK_TOKEN` dari pemilik**; kode siap sejak 11b/13b, provider ditukar ke Xendit di 25a) | T | T | – |
...
|---|---|---|
| 1 | **Monetisasi** | Payment gateway + dunning (begitu kunci Xendit diserahkan; sementara itu: halaman langganan + pembayaran manual, quick wins penjualan) |
