...
/**
 * `updatedAt` ditambahkan Fase 22b. Sejak kurs bisa disegarkan cron dari sumber
 * luar, pemilik berhak melihat KAPAN angka itu berubah — pekerjaan latar yang
 * menulis ulang masukan akuntansi tanpa jejak di layar adalah persis pola yang
 * berulang kali jadi masalah di repo ini.
 */
export type ApiCurrency = { code: string; name: string; rate: number; isBase: boolean; updatedAt: string | null };
...
export const priceGroupSchema = z.object({
  name: z.string().trim().min(2, "Nama grup minimal 2 karakter").max(60),
});
export type PriceGroupInput = z.infer<typeof priceGroupSchema>;

/**
 * Satu harga khusus dalam daftar harga sebuah grup.
 *
 * `unitPrice` per satuan DASAR produk (lihat migrasi `0046`), dan 0 sah —
 * artinya gratis. Menghapus harga khusus dilakukan lewat DELETE, bukan dengan
 * mengirim 0.
 */
export const priceGroupItemSchema = z.object({
  unitPrice: z.number().int("Harga harus bilangan bulat").min(0, "Harga tidak boleh negatif").max(1_000_000_000_000),
});
export type PriceGroupItemInput = z.infer<typeof priceGroupItemSchema>;

export type ApiPriceGroupItem = {
  productId: string;
  sku: string;
  name: string;
  unit: string;
  /** Harga dasar produk, untuk pembanding di layar. */
  sellPrice: number;
  /** Harga khusus grup, per satuan dasar. */
  unitPrice: number;
};

/** Dari mana harga sebuah baris berasal — dipakai layar untuk melabeli barisnya. */
export type SumberHarga = "grup" | "dasar";

export type HasilHargaGrup = {
  /** Harga per satuan yang DIINPUT, siap dipakai sebagai `unitPrice` baris. */
  harga: number;
  sumber: SumberHarga;
};

/**
 * Pilih harga jual sebuah baris: harga khusus grup pelanggan bila ada, kalau
 * tidak harga dasar produk.
 *
 * Dua hal yang paling mudah salah, dan keduanya tidak terlihat salah di layar:
 *
 * **Rp 0 adalah harga, bukan "kosong".** Barang bonus memang berharga nol.
 * Ketiadaan harga khusus diwakili oleh `hargaGrup` yang `null`/`undefined`
 * (barisnya tidak ada di `price_group_items`), BUKAN oleh nilai 0. Menyamakan
 * keduanya membuat barang bonus diam-diam tertagih harga normal.
 *
 * **Arah konversi satuan.** `price_group_items.unit_price` disimpan per satuan
 * DASAR, sedangkan `invoice_lines.unit_price` per satuan yang DIINPUT. Jadi di
 * sini harganya DIKALI `faktor` — pasangan terbalik dari pembagian di
 * `konversiSatuanBaris()`. Salah arah membuat faktur per dus tertagih 1/24
 * harganya, dan tidak ada satu angka pun di layar yang terlihat ganjil.
 */
export function hargaUntukGrup(input: {
  hargaDasar: number;
  hargaGrup?: number | null;
  satuan?: SatuanBaris;
  faktor?: number;
}): HasilHargaGrup {
  const adaHargaGrup = input.hargaGrup !== null && input.hargaGrup !== undefined;
  const dasar = adaHargaGrup ? input.hargaGrup! : input.hargaDasar;
  const sumber: SumberHarga = adaHargaGrup ? "grup" : "dasar";

  // Faktor cacat (0, negatif, pecahan, NaN) diperlakukan 1 — sama seperti
  // `konversiSatuanBaris()`, supaya kedua sisi konversi tidak pernah memakai
  // faktor yang berbeda untuk baris yang sama.
  const faktorBersih = Math.trunc(Number(input.faktor));
  const faktor =
    input.satuan === "besar" && Number.isFinite(faktorBersih) ? Math.max(1, faktorBersih) : 1;

  return { harga: dasar * faktor, sumber };
}
