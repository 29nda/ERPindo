...
describe("hitungProrata (Fase 20k)", () => {
  ...
  // Fase 24: pelanggan yang belum berlangganan tidak punya periode berjalan,
  // jadi tidak ada yang bisa diprorata. Fungsi ini menandainya dengan
  // `bisaProrata: false` dan `bayarSekarang: 0` — BUKAN harga penuh, karena
  // penagihannya memang bukan urusan fungsi ini melainkan checkout biasa.
  // Mengembalikan harga penuh di sini akan membuat pemanggilnya menagih dua
  // kali: sekali lewat "prorata", sekali lagi lewat checkout.
  it("tanpa periode berjalan: bisaProrata false dan tidak ada yang ditagih di sini", () => {
    const r = hitungProrata({
      planSekarang: "starter",
      planBaru: "business",
      berakhirPada: null,
